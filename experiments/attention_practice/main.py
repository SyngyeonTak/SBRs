...
parser.add_argument('--hiddenSize', type=int, default=2, help='hidden state size')
...
def main():
    ...
    for epoch in tqdm(range(opt.epoch), desc = "Training Progress"):
        ...
        if epoch == (opt.epoch - 1):
            is_last_epoch = True
            print('is_last_epoch = True')
        else: 
            is_last_epoch = False
            print('is_last_epoch = False')

        hit, mrr = train_test(model, train_data, test_data, is_last_epoch)
        flag = 0
